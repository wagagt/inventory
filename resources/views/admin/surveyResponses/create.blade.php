...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.surveyResponse.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.survey-responses.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('survey_detail') ? 'is-invalid' : '' }}" type="number" name="survey_detail" id="survey_detail" value="{{ old('survey_detail', '') }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('survey') ? 'is-invalid' : '' }}" name="survey_id" id="survey_id">
                    @foreach($surveys as $id => $survey)
                        <option value="{{ $id }}" {{ old('survey_id') == $id ? 'selected' : '' }}>{{ $survey }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('responder') ? 'is-invalid' : '' }}" name="responder_id" id="responder_id">
                    @foreach($responders as $id => $responder)
                        <option value="{{ $id }}" {{ old('responder_id') == $id ? 'selected' : '' }}>{{ $responder }}</option>
                    @endforeach
                </select>
