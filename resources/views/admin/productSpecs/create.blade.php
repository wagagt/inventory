...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.productSpec.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.product-specs.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('inputy_type') ? 'is-invalid' : '' }}" name="inputy_type" id="inputy_type">
                    ...
                    @foreach(App\Models\ProductSpec::INPUTY_TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('inputy_type', 'text') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
