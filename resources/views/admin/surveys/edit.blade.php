    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.surveys.update", [$survey->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $survey->name) }}">
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('description') ? 'is-invalid' : '' }}" type="text" name="description" id="description" value="{{ old('description', $survey->description) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('date_start') ? 'is-invalid' : '' }}" type="text" name="date_start" id="date_start" value="{{ old('date_start', $survey->date_start) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('date_end') ? 'is-invalid' : '' }}" type="text" name="date_end" id="date_end" value="{{ old('date_end', $survey->date_end) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('ubication') ? 'is-invalid' : '' }}" name="ubication_id" id="ubication_id">
                    @foreach($ubications as $id => $ubication)
                        <option value="{{ $id }}" {{ (old('ubication_id') ? old('ubication_id') : $survey->ubication->id ?? '') == $id ? 'selected' : '' }}>{{ $ubication }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('user') ? 'is-invalid' : '' }}" name="user_id" id="user_id">
                    @foreach($users as $id => $user)
                        <option value="{{ $id }}" {{ (old('user_id') ? old('user_id') : $survey->user->id ?? '') == $id ? 'selected' : '' }}>{{ $user }}</option>
                    @endforeach
                </select>
                @if($errors->has('user'))
