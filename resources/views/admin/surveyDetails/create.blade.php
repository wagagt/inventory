...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.surveyDetail.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.survey-details.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('answer_type') ? 'is-invalid' : '' }}" name="answer_type_id" id="answer_type_id">
                    @foreach($answer_types as $id => $answer_type)
                        <option value="{{ $id }}" {{ old('answer_type_id') == $id ? 'selected' : '' }}>{{ $answer_type }}</option>
                    @endforeach
                </select>
