    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.transactions.update", [$transaction->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('description') ? 'is-invalid' : '' }}" name="description" id="description">{!! old('description', $transaction->description) !!}</textarea>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $transaction->name) }}" required>
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('store_origin') ? 'is-invalid' : '' }}" type="number" name="store_origin" id="store_origin" value="{{ old('store_origin', $transaction->store_origin) }}" step="1">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('store_destiny') ? 'is-invalid' : '' }}" type="number" name="store_destiny" id="store_destiny" value="{{ old('store_destiny', $transaction->store_destiny) }}" step="1">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('customer') ? 'is-invalid' : '' }}" type="number" name="customer" id="customer" value="{{ old('customer', $transaction->customer) }}" step="1">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('provider') ? 'is-invalid' : '' }}" type="number" name="provider" id="provider" value="{{ old('provider', $transaction->provider) }}" step="1">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status_id" id="status_id">
                    @foreach($statuses as $id => $status)
                        <option value="{{ $id }}" {{ (old('status_id') ? old('status_id') : $transaction->status->id ?? '') == $id ? 'selected' : '' }}>{{ $status }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('type') ? 'is-invalid' : '' }}" name="type_id" id="type_id">
                    @foreach($types as $id => $type)
                        <option value="{{ $id }}" {{ (old('type_id') ? old('type_id') : $transaction->type->id ?? '') == $id ? 'selected' : '' }}>{{ $type }}</option>
                    @endforeach
                </select>
                @if($errors->has('type'))
