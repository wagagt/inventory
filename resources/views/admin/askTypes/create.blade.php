...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.askType.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.ask-types.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('ask') ? 'is-invalid' : '' }}" name="ask_id" id="ask_id">
                    @foreach($asks as $id => $ask)
                        <option value="{{ $id }}" {{ old('ask_id') == $id ? 'selected' : '' }}>{{ $ask }}</option>
                    @endforeach
                </select>
                @if($errors->has('ask'))
